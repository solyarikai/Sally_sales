...
class CRMSyncService:
    ...
    async def sync_smartlead_replies(
        ...
        Uses the statistics endpoint to find ALL leads with reply_time set.
        For each new reply: enrich via global lead search, fetch message
        history for actual reply text, strip HTML, run through reply pipeline.
        """
        if not self.smartlead:
            raise ValueError("Smartlead API key not configured")
            ...
            for campaign in campaigns:
                ...
                try:
                    ...
                    for reply_data in replied_leads:
                        ...
                        # Fetch message history for reply content
                        reply_body = ""
