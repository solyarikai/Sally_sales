...
async def list_replies(
    ...
    # needs_followup: show parents that have a pre-generated pending follow-up draft.
    # Like regular replies, drafts are created at detection time (by scheduler every 3 min).
    # The tab only shows cards with ready-to-send drafts — no "Generate" button needed.
    if needs_followup:
        from sqlalchemy.orm import aliased
        fu_max_age = datetime.utcnow() - timedelta(days=60)
        conditions.append(ProcessedReply.parent_reply_id.is_(None))  # Only parents
        conditions.append(ProcessedReply.approval_status == "approved")  # Only operator-sent replies (not auto_resolved)
        conditions.append(ProcessedReply.approved_at > fu_max_age)
        conditions.append(ProcessedReply.received_at > fu_max_age)  # Skip ancient conversations
        # MUST have a pending child draft (generated by scheduler)
        fu_child_pending = aliased(ProcessedReply)
        conditions.append(
            exists(
                select(fu_child_pending.id).where(
                    fu_child_pending.parent_reply_id == ProcessedReply.id,
                    fu_child_pending.approval_status.is_(None),
                )
            )
        )
        # Exclude if a follow-up was already sent or dismissed
        fu_child_done = aliased(ProcessedReply)
        conditions.append(
            ~exists(
...
async def get_reply_counts(
    ...
    if needs_followup:
        # Count parents with pre-generated pending follow-up drafts
        from sqlalchemy.orm import aliased
        fu_max_age = datetime.utcnow() - timedelta(days=60)
        base.append(ProcessedReply.parent_reply_id.is_(None))
        base.append(ProcessedReply.approval_status == "approved")  # Only operator-sent replies
        base.append(ProcessedReply.approved_at > fu_max_age)
        base.append(ProcessedReply.received_at > fu_max_age)
        # Must have pending child draft
        fu_child_pending = aliased(ProcessedReply)
        base.append(exists(
            select(fu_child_pending.id).where(
                fu_child_pending.parent_reply_id == ProcessedReply.id,
