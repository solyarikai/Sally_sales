...
description: "Создаёт Apollo-фильтры для нового ICP-сегмента через структурированный диалог. Используй ВСЕГДА когда пользователь говорит: \"создай фильтры для нового сегмента\", \"build Apollo filters\", \"нужны фильтры Аполло\", \"добавить сегмент в пайплайн\", \"собери фильтры для [название сегмента]\", \"новый сегмент для [проект]\". Задаёт по одному вопросу с контекстом, вариантами A/B/C и рекомендацией. На выходе: filter JSON + обновлённая документация."
---
...
**Если файла НЕТ** — это первый запуск. Поприветствуй пользователя и объясни как работает скилл:

> **Привет! Это первый запуск apollo-segment-builder.**
>
> Вот как это работает:
>
> Я буду задавать тебе **6 вопросов по одному** — по каждому параметру Apollo-фильтра. К каждому вопросу я даю контекст, три варианта ответа (A/B/C) и свою рекомендацию. Ты отвечаешь одной буквой.
>
> На выходе ты получишь:
> - готовый JSON-файл фильтров для пайплайна
> - обновлённую документацию по сегментам
> - чеклист что ещё нужно сделать вручную
>
> После каждого сегмента я буду обновлять базу знаний — и со временем смогу принимать часть решений самостоятельно.
>
> Сейчас базы знаний ещё нет — сначала поищу существующие файлы с фильтрами в проекте, чтобы не начинать с нуля.

Затем запусти bootstrap (шаг 0.3).

### 0.2 Сбор контекста сегмента

Если аргументы не переданы, спроси одним блоком (не по одному):

> Чтобы начать, мне нужно несколько вещей:
> 1. **Название сегмента** — slug на английском, напр. `SOCIAL_COMMERCE`
> 2. **Описание** — что делают компании этого сегмента (1-2 предложения)
> 3. **Куда сохранять результаты?** — укажи папку или путь. Если не знаешь — просто напиши `.` и я сохраню рядом с текущей директорией.

После получения ответов — проверь есть ли в указанной папке (или рядом) существующий файл с фильтрами других сегментов. Если есть — прочитай для понимания контекста и избежания overlap. Если нет — всё будет создано с нуля.

Сохраняй ответы пользователя в памяти по ходу диалога.

### 0.3 Bootstrap базы знаний (только если файл не существует)

Найди существующие файлы с фильтрами — ищи от текущей рабочей директории вниз по дереву.
Не знаешь структуру проекта — не угадывай пути. Используй поиск:

- Файлы с "filter" или "filters" в имени (`.md`, `.json`)
- Файлы с "segment" в имени (`.md`)
- Файлы с "apollo" в имени (`.md`, `.json`)

Спроси пользователя только если ничего не нашёл:
> "Не нашёл файлов с фильтрами. Есть ли у тебя существующие описания сегментов или фильтров? Укажи путь или вставь содержимое."

Из найденных файлов извлеки и структурируй:
...
## Режим работы: диалог vs автономный

После чтения базы знаний — оцени насколько она заполнена.

**Считай базу достаточной если:**
- В ней зафиксированы решения по **3 и более сегментам**
- По каждому параметру (geography, size, industry, keywords, titles) есть хотя бы 2 прецедента с обоснованием
- Есть данные о конверсиях или валидации (раздел "Validated Data")

**Если база достаточная — предложи автономный режим:**

> "У меня достаточно данных чтобы собрать фильтры самостоятельно. Могу предложить готовый вариант без вопросов — ты просто подтвердишь или скорректируешь итог.
> Запустить автономно? (да / нет, хочу отвечать на каждый вопрос)"

**Автономный режим:**
Прими все решения самостоятельно на основе базы знаний. Выведи полный черновик фильтров одним блоком с кратким обоснованием каждого решения. Попроси одно подтверждение перед сохранением:
> "Вот что я собрал. Всё окей или что-то правим?"

После подтверждения — переходи к шагу 8.

**Если база недостаточная — диалог (шаги 1-6).**
По каждому вопросу где в базе есть чёткий прецедент — можешь сразу назвать рекомендацию как "очевидную" и сократить описание вариантов. Где прецедентов нет — развёрнутый вопрос как обычно.
