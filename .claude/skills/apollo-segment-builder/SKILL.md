...
### 0.1 Проверь базу знаний

Первым делом — проверь существует ли `references/filter-decisions.md` рядом со скиллом.

**Если файл существует** — прочитай его и используй как основу для рекомендаций в диалоге. Переходи к 0.2.

**Если файла НЕТ** — запусти bootstrap (шаг 0.3) перед началом диалога. Сообщи пользователю:
> "База знаний не найдена. Сейчас соберу её из существующих фильтров — это займёт минуту."

### 0.2 Сбор контекста сегмента
...
### 0.3 Bootstrap базы знаний (только если файл не существует)

Найди и прочитай все доступные источники фильтров в проекте:

**Приоритет источников:**
1. `sofia/projects/*/docs/apollo-filters-v4.md` — последняя версия фильтров со всеми сегментами
2. `sofia/projects/*/docs/apollo-filters-v3.md` — предыдущая версия с примечаниями по валидации
3. `magnum-opus/scripts/sofia/filters/*.json` — все JSON filter files
4. `sofia/projects/*/docs/segment-*.md` — segment-doc файлы с обоснованием сегментов

Из этих файлов извлеки и структурируй:
- **По каждому существующему сегменту:** какие geography/size/industry/keywords/exclusions/titles были выбраны
- **Почему:** любые комментарии, сноски, пометки "validated by Sally's data", "0 конверсий", "strong fit"
- **Что менялось от версии к версии** (v3→v4) и почему

Затем создай `references/filter-decisions.md` используя структуру из шаблона ниже, заполнив таблицы реальными данными из найденных файлов. Незаполненные разделы (например, нет данных о конверсиях) — оставь с пометкой `_данных пока нет_`.

После создания файла — сообщи пользователю что база знаний инициализирована и перечисли какие сегменты в неё попали. Переходи к шагу 0.2.

**Шаблон структуры `filter-decisions.md`:**
```
# Apollo Filter Decision Knowledge Base
> Инициализирована: YYYY-MM-DD. Источники: [перечисли файлы]

## 1. Geography
### Стандартные блоки стран
[таблица блоков]
### Решения по сегментам
[таблица: Сегмент | Выбор | Обоснование]

## 2. Company Size
[таблица: Сегмент | Диапазон | Обоснование]

## 3. Industry
[таблица: Сегмент | Комбинация | Почему]

## 4. Keywords
[подразделы по сегментам с логикой выбора]

## 5. Excluded Keywords
[таблица сегмент-специфичных exclusions]

## 6. Job Titles
[таблица доп. титулов по сегментам]

## 7. Validated Data
[всё что найдено в комментариях фильтров о конверсиях]

## 8. TAM-ориентиры
[если есть данные по объёмам]
```
