# Fetch Unread Replies API Documentation

## Overview
Retrieves all unread replies in the master inbox using the same parameters and structure as the Inbox Replies endpoint.
...
**Endpoint Path:** `/unread-replies`

**Full URL:** `https://server.smartlead.ai/api/v1/master-inbox/unread-replies`
...
| `filters.campaignId` | array(integer) | No | - | Filter by campaign ID (array of numbers, max 5) |
| `filters.emailAccountId` | array(integer) | No | - | Filter by email account ID (array of numbers) |
| `filters.campaignTeamMemberId` | array(integer) | No | - | Filter by team member ID (array of numbers) |
| `filters.campaignTagId` | array(integer) | No | - | Filter by campaign tag ID (array of numbers) |
| `filters.campaignClientId` | array(integer) | No | - | Filter by client ID (array of numbers) |
...
```bash
curl -X POST "https://server.smartlead.ai/api/v1/master-inbox/unread-replies?api_key=YOUR_API_KEY&fetch_message_history=true" \
