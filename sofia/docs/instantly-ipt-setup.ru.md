...
| Хочу | Бери |
|---|---|
| Постоянный мониторинг, минимум возни | [Способ 1 — Промпт 1](#промпт-1--настроить-cron-мониторинг-tuefri). Одна команда Claude → cron + скрипты + Slack развёрнуты автоматически. Кастомный формат отчёта (бакеты, фильтр foreign senders). |
...
| Без Claude Code / без доступа к Hetzner — только UI | [Способ 2 — UI (Шаги 1–6)](#способ-2-через-ui-appinstantlyai) для разового теста, или [Шаг 7 (Automated test + Slack)](#шаг-7-альтернатива-automated-test--slack-вместо-cron-скриптов) для рекуррентного. Возни больше: каждый раз руками заполнять форму, настраивать Slack-интеграцию. |
