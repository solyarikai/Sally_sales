...
| Хочу | Бери |
|---|---|
| Постоянный мониторинг, минимум возни, кастомный формат отчёта | [Способ 1 — Промпт 1](#промпт-1--настроить-cron-мониторинг-tuefri). Одна команда Claude → cron + скрипты + Slack развёрнуты автоматически. Бакеты Healthy/Problematic/Silent, фильтр foreign senders. |
