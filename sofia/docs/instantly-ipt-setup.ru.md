...
| Хочу | Бери |
|---|---|
| Постоянный мониторинг через Slack, минимум возни | [Способ 2 — UI, шаг 7 (Automated test + Slack)](#шаг-7-альтернатива-automated-test--slack-вместо-cron-скриптов). Instantly сам гоняет тест по расписанию. |
| Постоянный мониторинг с кастомной логикой отчёта (бакеты, фильтр foreign senders, наш формат сообщения) | [Способ 1 — Промпт 1](#промпт-1--настроить-cron-мониторинг-tuefri). Claude разворачивает cron + скрипты на Hetzner. |
| Один разовый тест прямо сейчас | [Способ 1 — Промпт 2](#промпт-2--одноразовый-ручной-тест) (через Claude) или [Способ 2 — UI до шага 6](#способ-2-через-ui-appinstantlyai) (руками). |
