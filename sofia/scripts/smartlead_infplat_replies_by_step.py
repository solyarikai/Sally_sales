#!/usr/bin/env python3
"""
SmartLead — INFPLAT Replies by Step
=====================================
Собирает все ответы по INFLUENCER PLATFORMS кампаниям.
Показывает: на какой шаг / subject ответили лиды.

Run: ssh hetzner "cd ~/magnum-opus-project/repo && set -a && source .env && set +a && python3 sofia/scripts/smartlead_infplat_replies_by_step.py"
"""
...
def main():
    ...
    print("\n=== OnSocial INFLUENCER PLATFORMS — Replies by Step ===\n")
    ...
    for campaign in ["Global", "MENA+APAC", "India", "Americas"]:
        ...
        for subject, leads in sorted(subjects.items(), key=lambda x: -len(x[1])):
            subj_display = subject[:80] if subject else "(empty / thread)"
    ...
    out_path = "sofia/projects/OnSocial/hub/infplat_replies_by_step.json"
