...
SMARTLEAD_EMAIL_ACCOUNTS = [
    15090446,
    15090416,
    15090400,  # bhaskar@ (OnSocial)
    12300705,
    12300692,
    12300668,  # petr@ crona-ai
    11812436,
    11812422,
    11812388,
    11812365,
...
PEOPLE_TITLES = [
    ...
    # v5 additions — engineering/technical decision-makers
    "CTO",
...
def save_state(run_id: int, phase: str, gate_id: int = None):
    save_json(
        RUN_STATE,
        {"run_id": run_id, "phase": phase, "gate_id": gate_id, "updated_at": ts()},
    )
...
def step0_start() -> int:
    ...
    print("  Source: clay.companies.emulator (description_keywords)")
    print(
        f"  Keywords: {len(CLAY_FILTERS['description_keywords'])} description_keywords"
    )
    print(f"  Excluded: {len(CLAY_FILTERS['description_keywords_exclude'])} keywords")
    print(f"  Industries: {', '.join(CLAY_FILTERS['industries'])}")
    print(
        f"  Employees: {CLAY_FILTERS['minimum_member_count']}-{CLAY_FILTERS['maximum_member_count']}"
    )
    print("  Geo: ALL (no country_names filter)")
...
def approve_pending_gate(run_id: int) -> bool:
    try:
        gates = api(
            "get",
            f"/pipeline/gathering/approval-gates?project_id={PROJECT_ID}",
            raise_on_error=False,
        )
...
def blacklist_approved_targets(run_id: int):
    sql = (
        f"SELECT DISTINCT dc.domain FROM discovered_companies dc "
    ...
    insert_sql = (
        f"INSERT INTO project_blacklist (project_id, domain, reason, source, created_at) "
...
def step5_analyze(run_id: int, prompt_text: str = None) -> dict:
    ...
    if pending:
        ...
        print("  Review targets, then approve or re-analyze.")
        return {
            "gate_id": gate["id"],
...
def step6_prepare_verify(run_id: int) -> dict:
    ...
    result = api(
        "post",
        f"/pipeline/gathering/runs/{run_id}/prepare-verification",
        raise_on_error=False,
    )
    gates = api("get", f"/pipeline/gathering/runs/{run_id}/gates", raise_on_error=False)
    gate_list = gates if isinstance(gates, list) else gates.get("items", [])
...
def step9_export_targets(force: bool = False) -> list[dict]:
    ...
    today = tag()
    save_csv(
        CSV_DIR / f"targets_{SEGMENT_CODE}_{today}.csv",
        targets,
...
def _run_apollo_people_api(
    ...
    try:
        ...
        print("    Apollo API timeout (3600s)")
        if Path(output_path).exists():
            with open(output_path) as f:
                return json.load(f)
    except Exception as e:
...
def _map_apollo_person(
    person: dict, targets_by_domain: dict, batch_domain: str = None
) -> dict:
    ...
    return {
        ...
        "employees": str(
            ...
            or person.get("organization", {}).get("estimated_num_employees", "")
            or target.get("employees", "")
        ),
...
def step10_apollo_people_search(targets: list[dict], force: bool = False) -> list[dict]:
    ...
    print(f"\n{'=' * 60}")
    print("STEP 10: Apollo People Search (Internal API)")
    print(f"{'=' * 60}")
    ...
    print(
        f"  Batch size: {APOLLO_PEOPLE_BATCH_SIZE}, max pages: {APOLLO_PEOPLE_MAX_PAGES}"
    )

    output_path = "/tmp/apollo_people_clay_imagency_v4_all.json"
    ...
    today = tag()
    save_csv(
        CSV_DIR / f"import_apollo_people_{today}.csv",
        all_contacts,
...
def step10_import_apollo_csv(
    csv_path: str, targets: list[dict], force: bool = False
) -> list[dict]:
    """Import contacts from a manual Apollo People CSV export."""
    print(f"\n{'=' * 60}")
    print("STEP 10: Import Apollo People CSV")
    print(f"{'=' * 60}")
    ...
    for row in raw_rows:
        ...
        domain = _normalize_domain(
            _get("domain")
            or (_get("email").split("@")[-1] if "@" in _get("email") else "")
        )
        target = targets_by_domain.get(domain, {})
...
def export_getsales(without_email: list[dict], today: str) -> Path:
    ...
    out_path = (
        gs_dir
        / f"GetSales - {SEGMENT_NAME}_without_email - {date_folder.replace('_', '.')}.csv"
    )
    gs_rows = []
...
async def step11_findymail(
    contacts: list[dict], max_contacts: int = 1500, force: bool = False
) -> list[dict]:
    print(f"\n{'=' * 60}")
    print("STEP 11: FindyMail Enrichment")
    print(f"{'=' * 60}")
...
def step12_upload(contacts: list[dict]):
    ...
    print("  Social proof distribution:")
    ...
    if not _checkpoint(f"Attach {len(SMARTLEAD_EMAIL_ACCOUNTS)} email accounts?"):
        ...
        r = httpx.post(
            ...
            params=sl_params(),
            json={"email_account_ids": SMARTLEAD_EMAIL_ACCOUNTS},
            timeout=30,
        )
        if r.status_code == 200:
    ...
    if not _checkpoint("Set schedule Mon-Fri 8am-6pm EST?"):
...
def main():
    p = argparse.ArgumentParser(
        description="OnSocial Clay IM_FIRST_AGENCIES v4 ALL GEO Pipeline"
    )
    p.add_argument(
        "--from-step", choices=STEPS, default="start", help="Start from this step"
    )
    ...
    print("  Company source: Clay (description_keywords, direct, FREE)")
    print("  People source: Apollo People UI (Puppeteer, FREE)")
    print(f"  description_keywords: {len(CLAY_FILTERS['description_keywords'])}")
    print(
        f"  description_keywords_exclude: {len(CLAY_FILTERS['description_keywords_exclude'])}"
    )
    print(f"  Industries: {len(CLAY_FILTERS['industries'])}")
    print(
        f"  Employees: {CLAY_FILTERS['minimum_member_count']}-{CLAY_FILTERS['maximum_member_count']}"
    )
    print("  Geo: ALL (no country_names)")
    ...
    if "blacklist" in steps and run_id:
        ...
        if phase == "awaiting_scope_ok":
            ...
            if pending:
                ...
                print("  PAUSING. Approve gate, then resume:")
    ...
    if "people" in steps:
        if args.apollo_csv:
            contacts = step10_import_apollo_csv(
                args.apollo_csv, targets, force=args.force
            )
        else:
    ...
    if "findymail" in steps:
        contacts = asyncio.run(
            step11_findymail(
                contacts, max_contacts=args.max_findymail, force=args.force
            )
        )
    else:
