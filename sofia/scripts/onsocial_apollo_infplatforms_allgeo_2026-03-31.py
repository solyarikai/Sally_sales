...
Usage (run on Hetzner via SSH):
  ...
  # Step 0: Scrape Apollo Companies (takes 30-60 min for 10K)
  python3 sofia/scripts/onsocial_apollo_infplatforms_allgeo_2026-03-31.py --from-step scrape-companies

  # Step 1+: Feed into backend and run full pipeline
  python3 sofia/scripts/onsocial_apollo_infplatforms_allgeo_2026-03-31.py --from-step feed

  # Resume from people search
  python3 sofia/scripts/onsocial_apollo_infplatforms_allgeo_2026-03-31.py --from-step people

  # Resume with manual CSV
  python3 sofia/scripts/onsocial_apollo_infplatforms_allgeo_2026-03-31.py --from-step people --apollo-csv export.csv
...
# State files
COMPANIES_RAW_FILE = STATE_DIR / "companies_raw.json"
...
PROJECT_CODE = "OS"
SEGMENT_NAME = "INFLUENCER_PLATFORMS"
SEGMENT_CODE = "INFPLAT"
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 0: APOLLO COMPANIES UI SCRAPE
# Runs apollo_universal_search.js via Puppeteer on Hetzner.
# Scrapes Apollo Companies tab for each keyword x size combination.
# FREE - no credits spent.
# ══════════════════════════════════════════════════════════════════════════════

def step0_scrape_companies(force: bool = False) -> list[dict]:
    ...
    if COMPANIES_RAW_FILE.exists() and not force:
        companies = load_json(COMPANIES_RAW_FILE)
        print(f"  Loaded from cache: {len(companies)} companies")
        return companies

    scraper_path = REPO_DIR / APOLLO_COMPANY_SCRAPER
    if not scraper_path.exists():
        scraper_path = Path(".") / APOLLO_COMPANY_SCRAPER
    if not scraper_path.exists():
        print(f"  ERROR: {APOLLO_COMPANY_SCRAPER} not found")
        print(f"  Tried: {REPO_DIR / APOLLO_COMPANY_SCRAPER}")
        sys.exit(1)
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if partial_path.exists():
                ...
                print(f"  Loaded partial results: {len(companies)} companies")
...
def step1_feed_to_backend(companies: list[dict], force: bool = False) -> list[int]:
    """Feed scraped company domains into backend gathering pipeline."""
    print(f"\n{'='*60}")
    ...
    if not _checkpoint(f"Create {len(batches)} gathering runs for {len(domains)} domains?"):
        sys.exit(0)
    ...
    save_json(STATE_DIR / "run_ids.json", run_ids)
    return run_ids
...
def step2_8_backend_pipeline(run_ids: list[int], prompt_text: str = None):
    """Process all runs through backend pipeline."""
    print(f"\n{'='*60}")
    print(f"STEPS 2-8: Backend Pipeline ({len(run_ids)} runs)")
    print(f"{'='*60}")

    _, prompt = get_latest_prompt()
    text = prompt_text or prompt
    if not text:
        print("  WARNING: No analysis prompt found. Runs will skip analyze step.")
...
async def step11_findymail(contacts: list[dict], max_contacts: int = 1500,
    ...
    print(f"  {len(to_enrich)} to enrich")
...
def step12_upload(contacts: list[dict]):
    """SmartLead upload. Adds leads to EXISTING campaign (not creating new)."""
    print(f"\n{'='*60}")
    ...
    cid = upload_log.get("campaign_id")

    if cid:
        print(f"\n  Existing campaign: {cid}")
    else:
        print(f"\n  No existing campaign. Enter campaign ID to add leads to,")
        print(f"  or press Enter to skip upload.")
        if sys.stdin.isatty():
            resp = input("  Campaign ID> ").strip()
            if resp.isdigit():
                cid = int(resp)
            else:
            ...
            print("  Non-interactive mode - skipping (no campaign ID).")
            return
    ...
    upload_log["campaign_id"] = cid
    upload_log["leads"] = uploaded
    upload_log["uploaded_at"] = ts()
    save_json(UPLOAD_LOG, upload_log)

    print(f"\n  Done. {uploaded} leads uploaded to campaign {cid}.")
...
STEPS = ["scrape-companies", "feed", "backend", "export", "people", "findymail", "upload"]
...
def main():
    ...
    print(f"  Filters: v4 ({len(COMPANY_KEYWORDS)} keywords, expanded titles)")
    print(f"  Company source: Apollo Companies UI (Puppeteer, FREE)")
    print(f"  People source: Apollo People UI (Puppeteer, FREE)")
    print(f"  Keywords: {len(COMPANY_KEYWORDS)} (v3: 17 + v4: 20 new)")
    print(f"  Sizes: {COMPANY_SIZES} (combined per keyword)")
    print(f"  Geo: {len(SOCIAL_PROOF_LOCATIONS)} social proof regions")
    ...
    # Step 0: Scrape companies from Apollo UI
    companies = []
    if "scrape-companies" in steps:
        companies = step0_scrape_companies(force=args.force)

    # Step 1: Feed to backend
    run_ids = load_json(STATE_DIR / "run_ids.json") or []
    if "feed" in steps:
        if not companies:
            companies = load_json(COMPANIES_RAW_FILE) or []
        if not companies:
            print("  ERROR: No companies. Run --from-step scrape-companies first.")
            sys.exit(1)
        run_ids = step1_feed_to_backend(companies, force=args.force)
    ...
    if "backend" in steps:
        if not run_ids:
            run_ids = load_json(STATE_DIR / "run_ids.json") or []
        if not run_ids:
            print("  ERROR: No run IDs. Run --from-step feed first.")
            sys.exit(1)
        step2_8_backend_pipeline(run_ids, prompt_text)
