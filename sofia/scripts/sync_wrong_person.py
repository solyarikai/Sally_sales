...
Usage:
  python3 sync_wrong_person.py --project OnSocial --campaign-id 3092917 --chat-id -100123456
  python3 sync_wrong_person.py --project EasyStaff --campaign-id 3050419 --chat-id -100789012

Flow:
  1. Query processed_replies for wrong_person + project filter
  2. Skip already-synced (tracked in contact_activities)
  3. Add to SmartLead campaign via SmartLead API directly
  4. Mark synced in contact_activities
...
def get_wrong_person_leads(project_filter: str):
    """
    Get unprocessed Wrong Person leads from processed_replies.
    Filtered by campaign_name ILIKE project_filter (e.g. '%OnSocial%').
    """
    conn = None
    try:
        ...
        cur.execute(query, (project_filter,))
        rows = cur.fetchall()
        cur.close()
...
def main():
    ...
    parser.add_argument('--campaign-id', required=True, type=int,
                        help='SmartLead campaign ID to add leads to')
    ...
    project_filter = f'%{args.project}%'
    campaign_id = args.campaign_id

    logger.info("=" * 60)
    logger.info(f"Starting Wrong Person sync")
    logger.info(f"Project: {args.project} (filter: campaign_name ILIKE '{project_filter}')")
    logger.info(f"Campaign ID: {campaign_id}")
    if args.dry_run:
        logger.info("DRY RUN - no changes will be made")
    logger.info("=" * 60)

    # 1. Get Wrong Person leads
    leads = get_wrong_person_leads(project_filter)
    logger.info(f"Found {len(leads)} unprocessed Wrong Person replies")

    if not leads:
        logger.info("No leads to process")
        if not args.dry_run:
            send_telegram_notification({'added': 0, 'failed': 0, 'errors': []}, 0, args.project, campaign_id, args.chat_id)
        return
    ...
    # 5. Send notification
    send_telegram_notification(stats, len(leads), args.project, campaign_id, args.chat_id)
