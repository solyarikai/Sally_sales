#!/usr/bin/env python3
"""
OnSocial Clay Pipeline (INFLUENCER_PLATFORMS v4, ALL GEO, 2026-03-31)
...
Filters from: sofia/projects/OnSocial/docs/apollo-filters-v4.md (Segment 1)
  - 34 description_keywords (v3: 17 + v4: 17 new adjacent keywords)
  - 36 description_keywords_exclude
  - Employees: 5-5,000
  - Industries: 5 (Computer Software, Internet, Marketing & Advertising, IT, Online Media)
...
People filters (v4):
  ...
  - Titles: 22 titles (v4: added Head of Data, VP Data, CDO, Head of Platform, VP Platform)
...
Usage (run on Hetzner via SSH):
  ...
  python3 sofia/scripts/onsocial_clay_infplat_v4_allgeo_2026-03-31.py --from-step start

  # Dry run (print filters, no API calls)
  python3 sofia/scripts/onsocial_clay_infplat_v4_allgeo_2026-03-31.py --dry-run

  # Resume from people search (auto Apollo)
  python3 sofia/scripts/onsocial_clay_infplat_v4_allgeo_2026-03-31.py --from-step people

  # Resume with manual CSV (fallback)
  python3 sofia/scripts/onsocial_clay_infplat_v4_allgeo_2026-03-31.py --from-step people --apollo-csv export.csv
...
STATE_DIR = SOFIA_DIR.parent / "state" / "onsocial" / "clay_infplat_v4_allgeo"
STATE_DIR.mkdir(parents=True, exist_ok=True)

CSV_DIR = SOFIA_DIR / "output" / "OnSocial" / "clay_infplat_v4_allgeo"
...
# ══════════════════════════════════════════════════════════════════════════════
# CLAY FILTERS (from apollo-filters-v4.md, Segment 1 — description_keywords)
# Uses description_keywords for direct Clay search — NO AI/Gemini mapping.
# ══════════════════════════════════════════════════════════════════════════════

CLAY_FILTERS = {
    "description_keywords": [
        # --- Original v3 keywords (17) ---
        ...
        "social data",
        # --- New v4 keywords (17 adjacent companies missed by v3) ---
...
def step10_apollo_people_search(targets: list[dict], force: bool = False) -> list[dict]:
    ...
    for batch_idx, batch_domains in enumerate(batches):
        ...
        output_path = f"/tmp/apollo_people_clay_infplat_v4_{batch_idx}.json"
...
CAMPAIGN_NAME = "c-OnSocial_INFLUENCER PLATFORMS v4 Clay ALL GEO #C"
...
def main():
    ...
    if "people" in steps:
        if not targets:
            targets = load_json(TARGETS_FILE) or []
        if not targets:
            print("  ERROR: No targets. Run --from-step export first.")
            sys.exit(1)
        if args.apollo_csv:
    ...
    if "findymail" in steps:
        ...
        if not contacts:
            print("  ERROR: No contacts. Run --from-step people first.")
            sys.exit(1)
    ...
    if "upload" in steps:
        ...
        if not contacts:
            print("  ERROR: No contacts. Run --from-step findymail first.")
            sys.exit(1)
