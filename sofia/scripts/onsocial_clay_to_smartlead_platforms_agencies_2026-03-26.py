...
# OAuth token path — checked in multiple locations
...
def _get_gsheets_creds():
    """Load Google OAuth credentials from token.json, refresh if expired."""
...
def _resolve_drive_folder(sheet_name: str) -> str | None:
    """Determine Drive folder ID from sheet name TYPE field.
    Convention: [PROJECT] | [TYPE] | [SEGMENT] — [DATE]
    """
    parts = sheet_name.split(" | ")
    if len(parts) >= 2:
        type_field = parts[1].strip()
        return GDRIVE_FOLDERS.get(type_field)
    return None
