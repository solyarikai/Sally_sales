/**
 * DEPRECATED — DO NOT INSTALL OR RUN.
 *
 * Replaced 2026-04-28 by sofia/scripts/blacklist_sync_smartlead.py
 * (SmartLead negative leads → project_blacklist on the DB), which is
 * the canonical blacklist writer for OnSocial.
 *
 * Why this script never worked:
 *   1. It looked for tabs with prefix "LEADS" in the OnSocial <> Sally
 *      spreadsheet. There are no such tabs (they are "Leads booking_Sofia",
 *      "LinkedIn Outreach", "Weekly replies"), so findLeadsSheets() always
 *      returned an empty array.
 *   2. Even if the prefix matched, it would have blacklisted EVERYONE we
 *      ever wrote to — not just the people who said "no". That would kill
 *      positive leads on re-targeting.
 *   3. The source of truth for "Not Interested / Do Not Contact" is in
 *      SmartLead, not in Sheets — so reading Sheets was the wrong source.
 *
 * Kept for git history. Filename intentionally prefixed with `_deprecated_`
 * so it cannot be picked up by Apps Script triggers anymore. If you find
 * an active trigger pointing here, delete it from
 * https://script.google.com/home/triggers.
 *
 * See: sofia/reports/OS_Ops_Blacklist_Sync_Fix_2026-04-28.md
 */
