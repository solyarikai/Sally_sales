...
═══════════════════════════════════════════════════════════
  ...
  Шаг 6:    VERIFY [ручной]
            Оператор с Claude проверяют REJECTED компании на false negatives.
            Цель — recall: найти максимум таргетов из входного массива.
            Смотреть на отклонённые: есть ли там компании, которые ДОЛЖНЫ быть таргетами?

  Шаг 7:    ADJUST PROMPT [ручной]
            Если промпт слишком строгий → правим → повторяем шаги 5-6.
            Цель: увеличить recall (больше таргетов из тех же компаний).
...
_AUTO_APPROVE = False  # Set via --auto-approve flag


def _checkpoint(message: str) -> bool:
    """Show checkpoint, wait for operator confirmation.

    Default: STOP and exit so operator can review before resuming with --from-step.
    With --auto-approve or interactive TTY: proceed inline.
    """
    print(f"\n  ★ CHECKPOINT: {message}")
    if sys.stdin.isatty():
        ...
        print("  Auto-approve mode — proceeding.")
        return True
    else:
        print("  PAUSED. Review results above, then resume with --from-step <next>.")
        print("  Use --auto-approve to skip checkpoints.")
        sys.exit(0)
...
def _get_run_domains(config: ProjectConfig, run_id: int) -> set[str] | None:
    ...
    sql = (
        f"SELECT LOWER(dc.domain) FROM discovered_companies dc "
        f"JOIN company_source_links csl ON csl.discovered_company_id = dc.id "
        f"WHERE csl.gathering_run_id = {run_id}"
    )
    try:
        r = subprocess.run(
            [
        ...
        print(f"  WARNING: Could not fetch run domains: {e}")
    return None
...
async def step10_findymail(
    ...
    if sys.stdin.isatty():
        ...
        print("  PAUSED. Review cost above, then resume with --from-step findymail.")
        print("  Use --auto-approve to skip checkpoints.")
        sys.exit(0)
...
def filter_existing_contacts(emails: list[str], project_id: int = 42) -> dict:
    ...
    for i in range(0, len(sanitized), 500):
        ...
        # Detect environment: on Hetzner or local
...
def upload_leads(campaign_id: int, contacts: list[dict]) -> int:
    ...
    all_emails = [c["email"].strip().lower() for c in contacts if c.get("email")]
    ...
    if blocked_emails:
        ...
        contacts = [
            c
            for c in contacts
            if c.get("email", "").strip().lower() not in blocked_emails
        ]
        print(
            f"  Filtered: {before} -> {len(contacts)} (removed {before - len(contacts)} existing contacts)"
        )
    leads = []
    for c in contacts:
        leads.append(
...
def _sync_contacts_to_backend(
    config: ProjectConfig, contacts: list[dict], campaign_name: str, segment: str
):
    """Immediately sync uploaded leads to backend contacts table.
    Ensures blacklist protection without waiting for CRM sync cycle."""
    bulk = []
    for c in contacts:
        ...
        bulk.append(
            {
                ...
                "job_title": c.get("job_title") or c.get("title"),
                "segment": segment,
                "project_id": config.project_id,
                "source": "pipeline_step12",
                "linkedin_url": c.get("linkedin_profile") or c.get("linkedin_url"),
    ...
    if not bulk:
        return
    result = api("post", "/contacts/bulk", json=bulk, raise_on_error=False)
    if isinstance(result, list):
        print(f"  Contacts synced to DB: {len(result)} records")
    elif isinstance(result, dict) and result.get("detail"):
        print(f"  ⚠ Contacts sync partial: {result['detail'][:100]}")
    else:
        print(f"  Contacts synced to DB: {len(bulk)} submitted")
...
def step12_smartlead(config: ProjectConfig, contacts: list[dict]):
    ...
    for seg_name, seg_contacts in sorted(by_segment.items()):
        ...
        if not _checkpoint(f"Upload {len(seg_contacts)} leads to '{campaign_name}'?"):
            ...
            # ── Sync contacts to backend DB (immediate blacklist protection) ──
            _sync_contacts_to_backend(config, seg_contacts, campaign_name, seg_name)
...
def main():
    ...
    p.add_argument(
        "--prompt-id",
        type=int,
        help="Use specific prompt ID from DB (overrides auto-select)",
    )
    ...
    if args.prompt_file:
        ...
        result = api(
            "get",
            f"/pipeline/gathering/prompts?project_id={config.project_id}",
            raise_on_error=False,
        )
        prompts = result if isinstance(result, list) else result.get("items", [])
        match = next((p for p in prompts if p["id"] == args.prompt_id), None)
        if match:
            ...
            print(f"  ERROR: prompt_id {args.prompt_id} not found in DB")
            import sys
    ...
    # --force OR --run-id always re-exports from DB (avoid stale targets.json from other runs)
    if "export" in steps or "verify" in steps or args.force or run_id:
        targets = step8_export_targets(config, force=True)
    else:
        targets = load_json(config.state_dir / "targets.json") or []

    # Filter targets to current run only (avoid re-processing old runs)
    if run_id and targets:
        run_domains = _get_run_domains(config, run_id)
        if run_domains:
            before = len(targets)
            targets = [
